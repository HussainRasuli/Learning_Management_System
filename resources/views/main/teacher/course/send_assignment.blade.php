...
<div class="col-md-12">
    <div class="alert alert-info" role="alert">
        ...
        <span id="alert-message" style="color:#4a4a4a;">The maximum uploadable file size is 50 MB.</span>
    </div>
    <input type="hidden" name="ma_id" id="ma_id" value="{{$ma_id}}">
    <input type="hidden" name="date" id="date" value="{{$date}}">
    <input type="hidden" name="relevant_week" id="relevant_week" value="{{$week}}">
    <input type="hidden" name="relevant_course" id="relevant_course" value="{{$co_id}}">

    <div id="drop-zone">
        <p class="mt-2">Drop File Here...</p>
        <div id="clickHere">Click me to select File <i class="mdi mdi-download-multiple"></i>
...
<script>
    ...
    var course = $('#relevant_course').val();
    ...
    function uploadFile(){
        ...
        if(_('select_file').files.length > 0){
            if(totalMB >= 50.0){
                ...
                for(var count = 0; count < _('select_file').files.length; count++)  
                {
                    if(!['application/pdf','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.openxmlformats-officedocument.presentationml.presentation','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'].includes(_('select_file').files[count].type))
                    {
        ...
        else
        {
            ...
            ajax_request.open("POST", "{{route('student_assignment')}}/" + course + "/" + week + "/" + date + "/" + ma_id + "/" + $('#sem-id').val()); // Variable Week comes from modules view
